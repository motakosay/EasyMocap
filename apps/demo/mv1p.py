...
def mv1pmf_skel(dataset, check_repro=True, args=None):
    ...
    kpts_repro = None
    for nf in tqdm(range(start, end), desc='triangulation'): #this loop processes each frame and shows a progress bar labeled "triangulation"
...
""".format(args.path, ', '.join(args.sub), args.out, 
    ...
    print(help)
    skel_path = join(args.out, 'keypoints3d') #skeleton_path = results/keypoints3d , no file or directory created here!
